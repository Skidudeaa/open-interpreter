...
class TestRawEvents:
    def test_insert_and_dedup(self, store):
        ...
        row1 = store.insert_raw_event(
            received_at_ms=1000, seq=1, session_id="s1",
            source_kind="hook", event_name="PreToolUse", payload=payload,
        )
        assert row1 is not None

        # Same payload → duplicate
        row2 = store.insert_raw_event(
            received_at_ms=1001, seq=2, session_id="s1",
            source_kind="hook", event_name="PreToolUse", payload=payload,
        )
        assert row2 is None
    ...
    def test_different_payloads_accepted(self, store):
        ...
        row1 = store.insert_raw_event(
            received_at_ms=1000, seq=1, session_id="s1",
            source_kind="hook", event_name="PreToolUse",
...
class TestAgents:
    def test_upsert_and_query(self, store):
        ...
        store.upsert_agent(
            "main:s1", "s1",
            agent_type="main", state="idle", state_source="observed",
        )
    ...
    def test_active_agents_filter(self, store):
        store.upsert_session("s1")
        store.upsert_agent("main:s1", "s1", agent_type="main", state="idle", state_source="observed")
        store.upsert_agent("sub:a1", "s1", agent_type="scout", state="finished", state_source="observed")
        store.upsert_agent("sub:a2", "s1", agent_type="surgeon", state="running_tool", state_source="observed")
...
class TestToolCalls:
    def test_insert_and_close(self, store):
        ...
        store.insert_tool_call(
            "tc1", "s1", "main:s1", "Read", 1000, input_preview="/tmp/foo.py",
        )

...
async_interpreter.server.run(port=8123)  # Default port is 8123, but you can customize it
```

## WebSocket API

### Establishing a Connection
Connect to the WebSocket server at `ws://localhost:8123/`.
...
### Modifying Settings
To change server settings, send a POST request to `http://localhost:8123/settings`. The payload should conform to [the interpreter object's settings](https://docs.openinterpreter.com/settings/all-settings).
...
response = requests.post("http://localhost:8123/settings", json=settings)
print(response.status_code)
```

### Retrieving Settings
To get current settings, send a GET request to `http://localhost:8123/settings/{property}`.

Example:
```python
response = requests.get("http://localhost:8123/settings/custom_instructions")
print(response.json())
...
openai.api_base = "http://localhost:8123/openai"  # Replace with your server URL if different
...
```bash
docker run -p 8123:8123 open-interpreter
```

This will expose the server on port 8123 of your host machine.

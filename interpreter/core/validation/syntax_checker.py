...
class SyntaxChecker:
    ...
    def _check_javascript(self, code: str, file_path: str) -> SyntaxCheckResult:
        ...
        try:
            ...
            return SyntaxCheckResult(
                valid=True, warnings=[f"Error running node: {e}"], language="javascript"
            )
        finally:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                return SyntaxCheckResult(
                    valid=False,
                    errors=[
                        SyntaxErrorInfo(0, 0, result.stderr.strip()[:500], file_path)
                    ],
                    language="typescript",
                )
            ...
            return SyntaxCheckResult(
                valid=True, warnings=[f"Error running tsc: {e}"], language="typescript"
            )
        finally:
            Path(temp_path).unlink(missing_ok=True)
    ...
    def _check_shell(self, code: str, file_path: str) -> SyntaxCheckResult:
        ...
        try:
            ...
            return SyntaxCheckResult(
                valid=True, warnings=[f"Error running bash: {e}"], language="shell"
            )
        finally:
            Path(temp_path).unlink(missing_ok=True)
